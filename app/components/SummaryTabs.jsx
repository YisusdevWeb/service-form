  ...
  return (
    <Box sx={{ maxWidth: 800, mx: "auto", p: 2 }}>
      <Typography
        variant="h5"
        gutterBottom
        sx={{ color: "#0f4c80", fontWeight: "bold", textAlign: "center" }}
      >
      ...
      {completedServices.map((service, index) => (
        <TabPanel key={service.uniqueServiceId} value={value} index={index}>
          <Paper
            sx={{ p: 3, mb: 3, backgroundColor: "#f5f5f5", borderRadius: 2 }}
          >
            <Typography
              variant="h6"
              sx={{ color: "#0f4c80", fontWeight: "bold" }}
            >
                ...
                <ul style={{ listStyleType: "none", paddingLeft: 0 }}>
                  {Object.entries(phaseSelections).map(([option, selected]) =>
                    selected && option !== "phaseTitle" ? (
                      <li
                        key={option}
                        style={{ color: "#333", marginBottom: "4px" }}
                      >
                        {option}
                      </li>
                    ) : null
