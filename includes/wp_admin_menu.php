...
function fsf_add_admin_menu() {
    ...
    add_submenu_page(
        null,
        __('User Details', 'funnel-services-form'),
        __('User Details', 'funnel-services-form'),
        'manage_options',
    ...
    add_submenu_page(
        'fsf-informacion-de-usuario',
        __('Privacy Policy Link Settings', 'funnel-services-form'),
        __('Privacy Policy Link Settings', 'funnel-services-form'),
        'manage_options',
        'fsf-form-settings',
...
function fsf_display_debug_logs_settings() {
    if (isset($_POST['fsf_debug_logs_nonce']) && wp_verify_nonce($_POST['fsf_debug_logs_nonce'], 'fsf_debug_logs')) {
        ...
        echo '<div class="notice notice-success is-dismissible"><p>' . __('Settings updated successfully!', 'funnel-services-form') . '</p></div>';
    }
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            ...
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="debug_enabled"><?php _e('Enable debug/logs mode', 'funnel-services-form'); ?></label></th>
                    <td><input name="debug_enabled" type="checkbox" id="debug_enabled" value="1" <?php checked($debug_enabled, '1'); ?> /> <?php _e('Show logs and errors on frontend', 'funnel-services-form'); ?></td>
                </tr>
            </table>
            <?php submit_button(); ?>
        </form>
    ...
    add_submenu_page(
        'fsf-informacion-de-usuario',
        __('Thank You Page Link', 'funnel-services-form'),
        __('Thank You Page Link', 'funnel-services-form'),
        'manage_options',
...
function fsf_display_thank_you_page_settings() {
    ...
    ?>
    <div class="wrap">
        <h1><?php _e('Thank You Page Link Settings', 'funnel-services-form'); ?></h1>
        <form method="post" action="">
            <?php wp_nonce_field('fsf_thanks_settings', 'fsf_thanks_settings_nonce'); ?>
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="thanks_url"><?php _e('Thank You Page URL', 'funnel-services-form'); ?></label></th>
...
function fsf_display_form_settings_page() {
    ...
    ?>
    <div class="wrap">
        <h1><?php _e('Privacy Policy Link Settings', 'funnel-services-form'); ?></h1>
        <form method="post" action="">
            <?php wp_nonce_field('fsf_form_settings', 'fsf_form_settings_nonce'); ?>
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="terms_url"><?php _e('Privacy Policy URL', 'funnel-services-form'); ?></label></th>
...
function fsf_display_user_info_page() {
    ?>
    <div class="wrap">
        <h1><?php _e('User Information', 'funnel-services-form'); ?></h1>
        <?php
        $args = array(
            'post_type' => 'user-info',
            'posts_per_page' => -1,
        ...
        <form method="post" action="<?php echo admin_url('admin-post.php?action=fsf_bulk_delete_user'); ?>">
            ...
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        ...
                        <th><?php _e('Name', 'funnel-services-form'); ?></th>
                        <th><?php _e('Email', 'funnel-services-form'); ?></th>
                        <th><?php _e('Phone', 'funnel-services-form'); ?></th>
                        <th><?php _e('Actions', 'funnel-services-form'); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($posts as $post) : ?>
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo admin_url('admin.php?page=fsf-detalles-usuario&post_id=' . $post->ID); ?>"><?php _e('View', 'funnel-services-form'); ?></a>
                                | 
                                <a href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=fsf_delete_user&post_id=' . $post->ID), 'fsf_delete_user_' . $post->ID); ?>" onclick="return confirm('<?php _e('Are you sure you want to delete this user?', 'funnel-services-form'); ?>');"><?php _e('Delete', 'funnel-services-form'); ?></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            ...
            <div style="margin-top: 10px;">
                <button type="submit" class="button button-primary" onclick="return confirm('<?php _e('Are you sure you want to delete the selected users?', 'funnel-services-form'); ?>');" style="margin-right: 10px;"><?php _e('Remove Selected', 'funnel-services-form'); ?></button>
                <button type="button" class="button button-secondary" id="export-selected" style="margin-left: 10px;"><?php _e('Export Selected', 'funnel-services-form'); ?></button>
            </div>
        </form>
...
function fsf_display_user_details_page() {
    ...
    if (!$post_id) {
        echo '<div class="notice notice-error"><p>' . __('Invalid post ID', 'funnel-services-form') . '</p></div>';
        return;
    }

    $post = get_post($post_id);
    if (!$post || $post->post_type !== 'user-info') {
        echo '<div class="notice notice-error"><p>' . __('Post not found', 'funnel-services-form') . '</p></div>';
        return;
    }
    ...
    ?>
    <div class="wrap">
        <h1><?php _e('User Data', 'funnel-services-form'); ?></h1>
        <table class="form-table">
            <tr>
                <th scope="row"><?php _e('Name', 'funnel-services-form'); ?></th>
            ...
            <tr>
                <th scope="row"><?php _e('Phone', 'funnel-services-form'); ?></th>
                <td><?php echo esc_html($whatsapp); ?></td>
            </tr>
            <tr>
                <th scope="row"><?php _e('Selected Services', 'funnel-services-form'); ?></th>
                <td>
                    <?php
                    if (is_array($services)) {
                        ...
                        _e('No services selected.', 'funnel-services-form');
                    }
                    ?>
                </td>
            </tr>
        </table>
        <a href="<?php echo admin_url('admin.php?page=fsf-informacion-de-usuario'); ?>" class="button"><?php _e('Back', 'funnel-services-form'); ?></a>
    </div>
    <?php
}
...
function fsf_delete_user() {
    if (!isset($_GET['post_id']) || !isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'fsf_delete_user_' . $_GET['post_id'])) {
        wp_die(__('Insufficient permissions to perform this action.', 'funnel-services-form'));
    }
    ...
    if ($post_id && get_post($post_id) && 'user-info' === get_post_type($post_id)) {
        ...
        wp_die(__('Invalid post.', 'funnel-services-form'));
    }
}
...
function fsf_export_user_info() {
    ...
    if (isset($_POST['post_ids'])) {
        ...
        if (is_array($post_ids)) {
            ...
            fputcsv($output, array('Name', 'Email', 'Phone', 'Selected Services'));
